...
export default function AddEditBoardModal({ type, setIsBoardModalOpen }) {
  ...
  const [newColumns, setNewColumns] = useState([
    { name: "À faire", tasks: [], id: uuidv4() },
    { name: "En cours", tasks: [], id: uuidv4() },
  ]);
  ...
  return (
    ...
    >
      <div className="modal">
        <h3>{type === "edit" ? "Edit" : "Créer Un"} Tableau</h3>
        <label htmlFor="board-name-input">Libélé du tableau</label>
        <div className="input-container">
          <input
            ...
            id="board-name-input"
            type="text"
            placeholder="e.g. RUS 331"
            className={!isValid && !name.trim() ? "red-border" : ""}
          />
          {!isValid && !name.trim() && (
            <span className="cant-be-empty-span text-L"> Ne peut être vide</span>
          )}
        </div>
        ...
        >
          {type === "add" ? "Créer un nouveau tableau" : "Sauvegarder les modifications"}
        </button>
      </div>
    </div>
  );
